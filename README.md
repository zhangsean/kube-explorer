...
- 优化 Pod CPU/RAM 资源用量展示，在蓝色实际用量进度条上标记 Req，并同时显示实际用量、Req、Lim 及各自已用比例；用量超出 Req 的 50%-150% 区间时警告，实际用量达到 Lim 的 90% 时突出显示容量风险。
- 压缩 Pod 列表的状态、名称、命名空间、就绪、重启、CPU/RAM、IP、节点和存活时间等列及列间距，为完整镜像名称提供更多显示空间。
- 重启次数支持悬停查看容器、退出码和最近重启时间，并结合 OOM、存活/启动探针 Event、上一次容器日志以及同节点批量退出特征显示更具体的中英文重启原因。
